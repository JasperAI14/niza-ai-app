...
export async function renderShareCard(opts: ShareCardOptions): Promise<Blob> {
  ...
  ctx.fillText("Niza Prime AI", PAD + 56, 80);
  ...
  if (truncated) {
    ...
    ctx.fillText("…continued in Niza Prime AI", PAD, y + 8);
  }
  ...
  ctx.textBaseline = "middle";
  ctx.fillText(opts.footer ?? "Created with Niza Prime AI", PAD, H - footerH / 2 + 6);
