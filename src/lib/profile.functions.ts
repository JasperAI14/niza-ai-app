...
function fallbackName(email: string | null) {
  if (!email) return "Niza Prime user";
...
async function aiReply(review: { rating: number; body: string; escalated: boolean }): Promise<string> {
  ...
  const prompt = `You are a professional customer support representative for Niza Prime AI. ${guidance}
...
Review: ${review.body}`;
  try {
    ...
    return review.escalated
      ...
      : "Thank you for taking the time to share your feedback. We truly appreciate your support, and your comments help us continue improving Niza Prime AI.";
  }
}
